...
FLASHMEM void handleCommand() {
  if (strncmp("stats", com_instance, maxLen) == 0) Serial.printf("Proc=%.1f%% (max %.1f%%), Mem16=%u (max %u), Mem32=%u (max %u)\n", AudioProcessorUsage(), AudioProcessorUsageMax(), AudioMemoryUsage(), AudioMemoryUsageMax(), AudioMemoryUsage_F32(), AudioMemoryUsageMax_F32());
  else if (strncmp("printLevels", com_instance, maxLen) == 0) printLevels = (com_pi == 0 ? true : com_params[0] != 0.0f);
  else if (strncmp("printTuner", com_instance, maxLen) == 0) printTuner = (com_pi == 0 ? true : com_params[0] != 0.0f);
